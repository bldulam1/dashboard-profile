...
function createScript(props) {
  ...
  const hilParameters = {
    ...
    SENSOR_FCR: {
      ...
      ECU_ALV_TO_OEM_BL:
        "C:/Gen12_SW/54051_Dai_Gen5/FBL/R2.6/Daimler_GEN5V2_DAI_FBL_1_31_50D2_6.cbt",
      ECU_OEM_TO_ALV_BL:
        "C:/Gen12_SW/54051_Dai_Gen5/FBL/R2.6/Daimler_GEN5V2_ALV_FBL_R255_31_44D24_0.odx"
    }
  };
  ...
  const dHilOut = Path.resolve("D:/HIL_out");

  const script = `Set-Location ${hilMasterLocation}; ./HILMaster.exe "${configINI}" "${hilParameter}" -o "${dHilOut}`;
...
function createHILTasks(props, files) {
  ...
  const promises = files.map(({ fileName, path, size }) => {
    ...
    const { script, hilParameters } = createScript({
      hilMasterLocation,
      ...props,
      fileName
    });
