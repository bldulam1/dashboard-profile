...
const location =
  "V:\\JP01\\DataLake\\Common_Write\\CLARITY_OUPUT\\Subaru_77GHz\\CVW_Conversion\\Brendon_Dulam\\20190918_103333\\MAT";
...
fs.readdir(location, (err, files) => {
  if (err) return console.error(err);
    ...
    files.forEach(file => {
      ...
      if (oldPath.includes("_split_F1")) {
        let newPath = oldPath.replace("_split_F1", "");
        fs.rename(oldPath, newPath, err => {
          err ? console.log(err) : console.log(newPath);
        });
      }
